...
<body id="app-container" class="menu-main-hidden show-spinner">
    ...
    <script>
        ...
        $(document).on('click', '#addtocart', function(e) {
            ...
            let getTotalPrice = 0;
            getTotalPrice = parseInt(total.val()) + parseInt(price);

            total.val(getTotalPrice)
            var list = `
        ...
        function changePrice(e) {
            ...
            let kalk = 0
            let prevTotal = $('#totalPrice')
            let getIdMenu = $('#' + getId).val()
            price = e.dataset.price
            if (getop == "kurang") {
                if (getIdMenu > 0) {
                    getIdMenu--
                    kalk = (parseInt(prevTotal.val()) - parseInt(price));
                    $('#' + getId + '').val(getIdMenu)
                }
            } else if (getop == "tambah") {
                getIdMenu++
                kalk = (parseInt(prevTotal.val()) + parseInt(price));
                $('#' + getId + '').val(getIdMenu)
            }
